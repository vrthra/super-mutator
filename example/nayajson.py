        ...
        while True:
            if isinstance(stack[(- 1)], list):
                    ...
                    if (last_token == '['):
                        if (token_type == TOKEN_TYPE.OPERATOR):
                            if (token == '{'):
                                ...
                                raise ValueError("Array must either be empty or contain a value.  Got '{}'".format(token))
                        else:
                            stack.append(token)
                    elif (last_token == ','):
                        if (token_type == TOKEN_TYPE.OPERATOR):
                        ...
                        if isinstance(stack[(- 1)], list):
                            ...
                            if isinstance(stack[(- 1)], list):
                                ...
                                raise ValueError("Array items must be followed by a comma or closing bracket.  Got '{}'".format(value))
                        else:
                            raise ValueError("Array items must be followed by a comma or closing bracket.  Got '{}'".format(value))
                    elif (last_token == '}'):
                    ...
                    if (last_token == '{'):
                        if (token_type == TOKEN_TYPE.OPERATOR):
                            if (token == '{'):
                                ...
                                raise ValueError("Object must either be empty or contain key value pairs.  Got '{}'".format(token))
                        ...
                        if isinstance(stack[(- 1)], list):
                            ...
                            if isinstance(stack[(- 1)], list):
                                ...
                                raise ValueError("Object key value pairs must be followed by a comma or closing bracket.  Got '{}'".format(value))
                    elif (last_token == ']'):
                ...
                if stack[(- 1)].set:
                    if (token_type == TOKEN_TYPE.OPERATOR):
                        if ((token != '}') and (token != ',')):
                            raise ValueError((("Object key value pairs should be followed by ',' or '}'.  Got '" + token) + "'"))
                        value = stack.pop()
                        if (len(stack) == 0):
                            return value
                        if isinstance(stack[(- 1)], list):
                        ...
                        raise ValueError((("Object key value pairs should be followed by ',' or '}'.  Got '" + token) + "'"))
                elif ((token_type == TOKEN_TYPE.OPERATOR) and (token == ':') and (last_type == TOKEN_TYPE.STRING)):
                    pass
                    ...
                    raise ValueError("Object keys must be separated from values by a single ':'.  Got '{}'".format(token))
            else:
                value = stack.pop()
                if isinstance(stack[(- 1)], list):
                    stack[(- 1)].append(value)
